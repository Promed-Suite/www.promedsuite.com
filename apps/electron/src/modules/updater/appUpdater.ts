...
const TIMESTAMP_STATUSES: UpdaterStatus[] = [
  "checking",
  "available",
  "downloading",
  "downloaded",
...
class AppUpdater {
  ...
  async checkForUpdates(): Promise<AppUpdateState> {
    ...
    this.checkingPromise = autoUpdater
      .checkForUpdates()
      .then((result) => {
        if (
          result?.updateInfo &&
          this.isNewerVersion(result.updateInfo.version)
        ) {
  ...
  hasUpdate(): boolean {
    ...
    return (
      ...
      this.isNewerVersion(this.state.info.version) &&
  ...
  private isNewerVersion(targetVersion: string): boolean {
    try {
      ...
      return (
        this.normalizeVersion(targetVersion) > this.normalizeVersion(current)
      );
    } catch {
  ...
  private setState(partial: Partial<AppUpdateState>) {
    const shouldStamp =
      partial.status && TIMESTAMP_STATUSES.includes(partial.status);
    this.state = {
      ...this.state,
      ...partial,
