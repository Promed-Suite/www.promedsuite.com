import { appProtocolName, isPackaged, mediaProtocolName } from "@/constants";
...
function handleCustomProtocol(
  protocol: Protocol = session.defaultSession.protocol
) {
...
async function handleColdStart() {
  ...
  try {
    const startupUrl = process.argv.find((arg) =>
      arg.startsWith(`${appProtocolName}://`)
    );
    if (startupUrl) {
...
async function dispatchDeepLink(
  url: string,
  source: "second-instance" | "open-url" | "cold-start"
) {
  try {
